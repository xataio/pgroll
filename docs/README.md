...
The `down` field above is required in order to backfill the previous version of the schema during an active migration. For instance, in our [example](../examples/09_drop_column.json), you can see that if a new row is inserted against thr new schema without a `price` column, the old schema `price` column will be set to `0`.
