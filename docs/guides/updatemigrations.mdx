...
We provide two ways to update your migration files. One way is to update your local migrations when you are in the middle of a development and you happened to update pgroll. Other method is to pull migration files from a remote PostgreSQL database, and update all migrations in the folder.

## Update your local migrations

Let's say that you are working on your latest feature, already written your migration file. Then you update `pgroll` to leverage the latest features and improvements of the fresh release. But the release also contains breaking changes that impact your local migrations. In this case you can run `update {migrations-folder}`, and `pgroll` will go through all of your local migrations and rewrite the operations that are no longer considered correct.
